...
class MainActivity : AppCompatActivity(), Robot.TtsListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.chipPisosHidraulicos.setOnClickListener { filterProducts("Pisos hidráulicos") }
            ...
            override fun onResults(results: Bundle?) {
                ...
                matches?.firstOrNull()?.let { command ->
                    when {
                        ...
                        command.contains("pisos", ignoreCase = true) -> filterProducts("Pisos hidráulicos")
                        else -> speak("Comando no reconocido.")
                    }
                }
                startListening()
            }
    ...
    private fun filterProducts(category: String) {
        ...
        OdooHelper.executeOdooRpc(
            ...
            onError = {
                speak("Error al cargar productos.")
            }
        )
    }
    ...
    private fun getStockForProduct(productId: Int, onSuccess: (Int) -> Unit) {
        val domain = listOf(
            listOf("product_id", "=", productId),
            listOf("location_id", "=", locationIdTienda)
        )
